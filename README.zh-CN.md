...
- **敏感条目**：正文里含**硬密钥**（手机号 / 身份证 / 银行卡 / `sk-…` / `Bearer …`）的条目会被打上 `sensitive`，此后**不再自动注入、也不会被检索返回**——只能显式 `mem_view <id>` 打开（`mem_list` 里会标 `[sensitive]`）。词级提及（`password`、`api_key`）**刻意不打标**：普通技术笔记里到处都是这两个词，按它们打标会让一大批记忆静默退出召回。
