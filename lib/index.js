...
/**
 * Tool results must be LOSSLESS JSON. DSH validates the value a tool returns, and an
 * own property whose value is `undefined` (or a non-finite number) does not survive a
 * JSON round-trip — the property vanishes, so the value is rejected and the caller sees
 * a failure even though the work succeeded. Tool bodies produce such fields naturally
 * (`{ superseded }` when nothing was superseded, `persistReason: undefined` on a clean
 * save), so results are normalized once here instead of at dozens of return sites.
 */
...
export function apply(ctx, config = {}) {
  ...
  // Single choke point: normalize every tool result to lossless JSON (see jsonSafe).
  const register = (tool) => ctx.effect(() => ctx.tools.register({
    ...tool,
    async execute(args, meta) {
      return jsonSafe(await tool.execute(args, meta))
    },
  }))
