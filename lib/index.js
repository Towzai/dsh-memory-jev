...
export const BUILD = 'memory-jev@0.1.0'
...
/**
 * Hard-secret subset: used to FLAG a stored entry so it is never auto-injected.
 * Deliberately excludes the word-level patterns (password / api_key): those appear
 * throughout ordinary technical notes, and flagging on them would silently drop a
 * large share of the library out of recall and injection.
 */
const HARD_SENSITIVE_PATTERNS = SENSITIVE_PATTERNS.slice(0, 5)

/** @param {string} text */
export function containsHardSecret(text) {
  if (!text) return false
  return HARD_SENSITIVE_PATTERNS.some(re => re.test(text))
}
...
export function normalizeEntry(entry) {
  ...
  // Entries imported from an older store carry no flag: scan once with the HARD patterns.
  if (typeof e.sensitive !== 'boolean') {
    e.sensitive = containsHardSecret([e.title, e.content, (e.tags ?? []).join(' ')].join(' '))
  }
  delete e.vector
  return e
}
...
export function createMemoryService(deps) {
  ...
  async function recall(args, meta = {}) {
    ...
    // Sensitive entries stay readable only through an explicit mem_view <id>.
    const active = store.active().filter(e => e.sensitive !== true)
    if (active.length === 0) {
    ...
    if (containsSensitive(query)) {
      log.append({ kind: 'recall-gate', gate: 'redacted-skip', session_id: meta.sessionId, result: { query_hash: createHash('sha256').update(query).digest('hex'), query_len: query.length } })
      return { gate: 'redacted-skip', degraded: { reason: 'sensitive' }, items: [] }
    }
    ...
    const items = rows.filter(r => r.jev_prob == null || r.jev_prob >= config.recallMinProbability).slice(0, limit)
    log.append({
  ...
  async function injectSnapshot(args) {
    ...
    const active = store.active()
    const pool = active.filter(e => !sess.injected.has(e.id) && e.sensitive !== true)
    if (pool.length === 0) {
      sess.silentTurns += 1
    ...
    if (containsSensitive(query.text)) {
      log.append({
